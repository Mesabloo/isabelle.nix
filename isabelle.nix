...
{ lib, stdenv, fetchurl, coreutils, nettools, jdk, polyml, z3_4_4_0, veriT, vampire, eprover-ho, naproche, rlwrap, perl, makeDesktopItem, isabelle-components, isabelle, symlinkJoin, curl, spass, leo2, leo3-bin, iprover, satallax, cvc4, cvc5, system, fetchhg }:
...
rec {
  ...
  postPatch = ''
    ...
    # rm -r heaps
  '' + ''
    arch=${if system == "x86_64-linux" then "x86_64-linux" else "x86-linux"}
    for f in contrib/*/$arch/{epclextract,nunchaku}; do
